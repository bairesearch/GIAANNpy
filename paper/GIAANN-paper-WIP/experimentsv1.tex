...
There is a tradeoff between recall/memorisation (train accuracy) and generalisation (test accuracy), and the optimum GIAANN parameters differ for both cases. 

GIAANN proto does not use word-vectors, which limits generalisation to words unseen in the test-set compared to contemporary gradient based models. GIAANN proto v1 also uses word instead of subword tokens.

Likewise, GIAANN relies on independent concept experts, which limits generalisation between concept columns. Although GIAANN is designed to support prime concept feature name independence (by detecting all columns matching a prompt), this is computationally expensive as it requires bursting matching features across the entire trained network during test time. Thus, test-set (prime) concept features must currently be present within the train-set. This limitation is strengthened by \texttt{enforceSequentialActivation} and \texttt{inferenceUseNeuronFeaturePropertiesTimeExact} in proto v1.
