...
Multi-compartment models typically also employ multiple dendritic branches, to process multiple input streams or conditions before firing \citep{leugering2020spiking, leugering2023dendritic} (see Simulated Dendritic Branches section~\ref{subsec:simulated-dendritic-branches} below).
...
A schematic of the cortical flow in the H01 Connectome is shown in \Cref{fig:intro-excitatory-cortical-assemblies}, generated by \citet{bairesearch2025LocalConnectome} using data from \citet{shapsonCoe2021petascale,shapsonCoe2024petavoxelScience}. See the BAI LocalConnectome repository for a detailed analysis of biological network connectivity across organisms \citep{bairesearch2025LocalConnectome}.
...
In GIAANN, this biological network and assembly concept is reflected in how neurons (within a concept column or across columns) are wired.
GIAANN emphasises excitatory-to-excitatory connections for driving the main signal flow (with inhibition playing a more modulatory role, such as resetting, normalising, or top-k selection).
...
\subsection{Simulated Dendritic Branches in Neural Models}\label{subsec:simulated-dendritic-branches}
...
Dendritic branches can act as non-linear subunits: they can individually perform thresholding and even spike on their own (dendritic spikes) before influencing the soma \citep{london2005dendriticComputation,poirazi2003two_layer}.
This means a single biological neuron can implement a multi-layer computation---effectively a small neural network within itself, where dendrites detect specific features and the soma integrates those results \citep{poirazi2003two_layer,london2005dendriticComputation}.
Pioneering theoretical work by \citet{poirazi2003two_layer} showed that a model pyramidal neuron can be abstracted as a two-layer neural network in which inputs are combined non-linearly within dendritic subunits before the branch outputs are integrated at the soma.
...
\citet{quiroga2005concept} famously reported neurons that respond selectively to different images of the same person (e.g. Jennifer Aniston), and in some cases also to the written name of that person.
...
The selectivity is highly sparse and explicit, with only a small fraction of neurons responding strongly to a given concept \citep{quianQuiroga2012conceptCells}.
This led to renewed discussion of ``grandmother cells''---not as evidence for a literal one-neuron-per-concept scheme, but as evidence that some neurons can occupy the highly explicit end of a sparse coding spectrum.
Additional research by \citet{quianQuiroga2012conceptCells} reinforced that concept cells are part of a distributed but sparse assembly coding for an item, and they likely play a role in memory (linking concepts together, since the hippocampal formation is involved in memory).
...
They also found that ensembles of such neurons encoded the hierarchical structure of semantic representations in their firing patterns \citep{jamali2024semantic_encoding}. 
...
Traditional deep networks, however, tend to rely primarily on distributed representations, although some individual units in trained models can show selectivity for particular high-level features or concepts.
...
This design is in line with the findings of concept cells \citep{quianQuiroga2012conceptCells} and semantic cells \citep{jamali2024semantic_encoding}.
...
It also allows contextual modulation of semantics: because GIAANN's neurons can interact (via lateral modulation), the activation of a concept neuron can reflect context, similar to \citeauthor{jamali2024semantic_encoding}'s observation that neuronal responses to words depend on sentence context.
...
\citeauthor{hawkins2011htm}' Hierarchical Temporal Memory (HTM) algorithm was an attempt to implement key aspects of cortical columns in a computational model.
...
\citeauthor{holtmaat2016assembly}’s assembly formation could be seen in the column context: if learning recruits an assembly of neurons, often those neurons are spatially proximate (due to local recurrent connectivity), possibly within a cortical column or a couple of neighboring columns \citep{holtmaat2016assembly}.
...
They describe multi-head attention as allowing the model to jointly attend to information from different representation subspaces at different positions, which lets different heads capture different relational aspects in parallel.
...
Auxiliary support for functional columnar processing can also be found in human language processing. Recent magnetoencephalography (MEG) analyses by \citet{zou2026constituent} suggest that sequence processing may be divided into chunks (GIA reference sets or GIAANN columns), and next-word statistical predictions do not apply uniformly within and across chunk boundaries (first token of a column).
...
GIAANN’s concept columns resonate with \citeauthor{hawkins2011htm}’ theory---each column holds a model of a piece of the world (like one facet of an object or one type of entity), and collectively these models contribute to a coherent perception or solution.
