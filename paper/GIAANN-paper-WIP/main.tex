\documentclass[11pt]{article}

\usepackage[T1]{fontenc}
\usepackage[utf8]{inputenc}
\usepackage{lmodern}
\usepackage{microtype}
\usepackage{amsmath,amssymb}
\usepackage{graphicx}
\usepackage{booktabs}
\usepackage{svg}
\usepackage{caption}
\captionsetup[figure]{name=Figure,labelsep=colon}
\usepackage{fancyvrb}
\usepackage[section]{placeins}
\usepackage{float}
\usepackage{hyperref}
\usepackage{xurl}
\usepackage[nameinlink,noabbrev]{cleveref}
\usepackage[numbers]{natbib}
\usepackage[scaled=0.7]{beramono}
\usepackage{listings}

\urlstyle{tt}
\makeatletter
\g@addto@macro\UrlBreaks{\do\=\do\-\do\_\do\:\do\/\do\?\do\&\do\,\do\.\do\+}
\makeatother

\DeclareRobustCommand{\code}[1]{\begingroup\Urlmuskip=0mu plus 1mu\relax\nolinkurl{#1}\endgroup}
\renewcommand{\texttt}[1]{\code{#1}}

\title{GIAANN Proto: A General Intelligence Algorithm Artificial Neural Network for Language Modeling}
\author{\textbf{Richard Baxter}\\\small BAI Research \\\small \texttt{richard.baxter@bairesearch.com.au}}
\date{\today}

\begin{document}
\maketitle

\begin{abstract}
General Intelligence Algorithm Artificial Neural Network (GIAANN) introduces an alternative learning algorithm to backpropagation: a dynamically growing network of features and connections. GIAANN Proto is an experimental implementation for language modeling.
The system constructs an excitatory database network of feature neurons organised into concept columns, incrementally updates that network during training, and performs autoregressive inference by predicting subsequent features conditioned on a seeded prompt. The neurons are sequentially activated and sensitive to the timing of their input (SANI). GIAANN provides advantages for biological feasibility, continual learning, online learning, catastrophic forgetting, concept-level generalisation, and robustness to hallucination. This paper summarises the core ideas and provides an implementation-oriented description intended to accompany the released code.
\end{abstract}

\section{Summary}
Neural language models are commonly trained with gradient-based backpropagation and fixed-context attention mechanisms.
GIAANN explores an alternative associative memory design: an online, dynamically growing network of features and connections intended to support continual learning and effectively unbounded context via a persistent database network.

This paper is a \emph{companion} to the codebase \texttt{github.com/bairesearch/GIAANNpy}.

\section{Background and Goals}
The project targets properties motivated by biological plausibility and continual learning, including online learning, continuous updates without catastrophic forgetting, a bias toward concept-level generalisation, and robustness to hallucination.

\input{intro}

\input{discussion}

\input{development}

\input{algorithm}

\input{implementation}

\input{experimentsv1}

\input{experimentsv2}


\section{Conclusion}
GIAANN Proto provides an experimental platform for exploring a persistent associative memory database-network approach to language modeling.
Future work should prioritise scaling studies, and further comparison to modern baselines. Additional exploration is likewise required to optimise the sequentially activated neuronal input parameters for test-set data, and the concept column segmentation. Finally, to move beyond self-supervised learning, reinforcement learning should be tested within the GIAANN architecture.

\section{Acknowledgements}

This paper was written with assistance from OpenAI GPT 5.2 Codex \citep{openai2025gpt52codex}, OpenAI Deep Research \citep{openai2025deepresearch}, and OpenAI Prism \citep{openai2026prism}.

\bibliographystyle{plainnat}
\bibliography{references}

\input{appendix}

\end{document}