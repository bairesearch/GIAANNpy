...
\paragraph{Hardware.}
ASUS B760 Intel Chipset, ASUS NVIDIA GeForce RTX 5070 (12GB GDDR7), NVIDIA Driver 580.126.09, CUDA 12.8. Intel i7 14700F. 64GB Patriot DDR4. Samsung 990 Pro PCIe 4.0 NVMe M.2 SSD (1TB).

\paragraph{Software.}
Ubuntu 24.04 LTS, IceWM, NEdit, Codex IDE extension for Visual Studio Code \citep{openai2026codexIdeExtension}.
...
\Cref{tab:oscar-train-top1-accuracy-spacyPipelineOptimisations} likewise contains OSCAR-2201 top-1 training-set accuracy for different numbers of segments and training budgets, with targets/predictions fed as input, and segment timing exact. It was trained with \url{spacyPipelineOptimisations=True} for efficiency.

\Cref{tab:oscar-train-top1-accuracy-useBenchmarkDefaultsFalse} contains OSCAR-2201 top-1 training-set accuracy for different numbers of segments, and different time constraints, with targets fed as input. The default time constraints used for benchmarking of the train-set(``t-1 exact'') are \texttt{inferenceUseNeuronFeaturePropertiesTimeExact=True} and \texttt{enforceSequentialActivation=True}. It also contains results for these non-default time constraints: \texttt{inferenceUseNeuronFeaturePropertiesTimeExact=False} and \texttt{enforceSequentialActivation=True} (``top-1 bias''), and \texttt{inferenceUseNeuronFeaturePropertiesTime=False} (``top-1 none''). It was trained with \texttt{useBenchmarkDefaults=False} (\url{spacyPipelineOptimisations=True} and \texttt{pretrainConceptColumnsDelimitByPOSenforce=True}) for efficiency.
...
\begin{table}[H]
\centering
\caption{OSCAR-2201 Top-1 training-set accuracy for different numbers of segments, and different time constraints, with targets fed as input}
