...
\paragraph{Hardware.}
ASUS PRIME B760-PLUS D4 (or non-D4). ASUS NVIDIA GeForce RTX 5070 (12GB GDDR7). Intel i7 14700F. 64GB Patriot 2666Mhz DDR4 (or 64GB Corsair 6000MT/s DDR5). 1TB Samsung 990 Pro PCIe 4.0 NVMe M.2 SSD.
...
\subsection{Results (proto v1)}

The accuracy measurements were taken with GIAANN Proto 1k (equivalent to 1m).
...
\Cref{tab:oscar-test-top1-accuracy-spacyPipelineOptimisations-inferenceConnectionStrengthTypes} contains OSCAR-2201 Top-1 test-set accuracy for different inference connection strength types, training sizes, and inference activations, with segment timing none (t-1 none), and targets fed as input. The different inference connection strength types used were ``bool'' (\texttt{inferenceConnectionsStrengthBoolean=True}) and ``float'' (\texttt{inferenceConnectionsStrengthBoolean=False}). The default strength type used during benchmarking is ``bool''.
...
8 & 3 & 5 & 3,000,000 & 0.153 & 0.142 & 986.1M & 69.4GB \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{OSCAR-2201 Top-1 test-set accuracy for different inference connection strength types, training sizes, and inference activations, with segment timing none (t-1 none), and targets fed as input.}
\label{tab:oscar-test-top1-accuracy-spacyPipelineOptimisations-inferenceConnectionStrengthTypes}
\begin{tabular}{rrrrrrrrr}
\toprule
\textbf{seed} & \textbf{c seg} & \textbf{f seg} & \textbf{sequences} & \textbf{strength} & \textbf{int f+c} & \textbf{bool f} & \textbf{param} & \textbf{db size} \\
\midrule
8 & 3 & 5 & 5,000     & bool & 0.029 & 0.029 & 2.2M & 0.1GB \\
8 & 3 & 5 & 200,000   & bool & 0.100 & 0.100 & 76.6M & 4.5GB \\
8 & 3 & 5 & 1,000,000 & bool & 0.134 & 0.130 & 353.7M & 20.3GB \\
8 & 3 & 5 & 3,000,000 & bool & 0.153 & 0.142 & 986.1M & 69.4GB \\
8 & 3 & 5 & 5,000     & float & 0.030 & 0.029 & 2.2M & 0.1GB \\
8 & 3 & 5 & 200,000   & float & 0.106 & 0.107 & 76.6M & 4.5GB \\
8 & 3 & 5 & 1,000,000 & float & 0.133 & 0.139 & 353.7M & 20.3GB \\
8 & 3 & 5 & 3,000,000 & float & 0.147 & 0.150 & 986.1M & 69.4GB \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Performance (proto v1)}

The performance measurements were taken with GIAANN Proto 1l/1m, after splitting the column souce feature connections tensors per source feature.

Experimental optimisations \texttt{optimisationGetFeatureConnectionsForSourceFeatureCache} and \texttt{optimisationObservedColumnsWriteMetadataCheck} were disabled for all measurements.

Time measurements were calculated across configurations. When \texttt{storeDatabaseFeatureConnectionsAndColumnFeatureNeuronsInRam=True}, time measurements for the sequence processing ($t\_proc$) were separated from the final write (train phase) or initial read (inference phase) of the entire database from/to CPU RAM ($t\_w/r$), where $t\_tot = t\_proc + t\_w/r$. 

GPU and CPU RAM measurements were calculated as peak memory allocated across execution.

The RAM database i/o device for features and connections was CPU for all measurements (\texttt{useGPUfileio=False}).

All measurements were conducted with \texttt{useGPUsparseStrict=True}. The primary effect of this setting is that if the sparse tensor device is set to CPU (\texttt{useGPUsparse=False}), it converts tensors to the sparse device (CPU) before converting tensors from dense to sparse. This can add some CPU memory overhead (memory/processing). This setting is not relevant for the measurements however because \texttt{useGPUsparse=True} by default, and there are no cases where dense tensors are used for training and \texttt{useGPUsparse=False}.

\paragraph{Tensor configurations}

\subparagraph{Sparsity}
All feature neuron and connection tensors are stored as sparse tensors (PyTorch COO format) in the database (RAM and filesystem). Dense tensors are predominately used during training under the default settings (\texttt{trainSparseConnectionsTensor=False} and \texttt{trainSparseNeuronsTensor=False}), and are never significantly used during inference. In the case where dense tensors are used during training, the database sparse feature neuron/connection tensors are updated with sequence specific trained connectivity information generated via dense matrix operations.

Sparse and dense tensor devices are by default both set to GPU during training (i.e. \texttt{useGPUsparse=True}/\texttt{useGPUdense=True}), and set to CPU during inference (i.e. \texttt{useGPUsparse=False}). 

\subparagraph{Database feature neurons}
If \texttt{storeDatabaseGlobalFeatureNeuronsInRam=True}, feature neurons sparse tensors are stored in a global tensor representing all columns. Else, feature neuron sparse tensors are stored independently for each column. 

Database global feature neuron sparse tensors are by default stored in RAM during training (i.e. \texttt{storeDatabaseGlobalFeatureNeuronsInRam=True}). Else if \texttt{storeDatabaseGlobalFeatureNeuronsInRam=False} and \texttt{storeDatabaseFeatureConnectionsAndColumnFeatureNeuronsInRam=True}: the column feature neurons are stored independently in RAM. Or if \texttt{storeDatabaseGlobalFeatureNeuronsInRam=False} and \texttt{storeDatabaseFeatureConnectionsAndColumnFeatureNeuronsInRam=False}: the filesystem database column feature neurons are read/written into/from RAM on a per sequence basis. 

Enabling global feature neuron tensors is mandatory during inference (i.e. \texttt{storeDatabaseGlobalFeatureNeuronsInRam=True}). If the train phase was conducted with \texttt{storeDatabaseGlobalFeatureNeuronsInRam=False}, either \texttt{trainEndGenerateGlobalFeatureNeuronsTensor=True} can be enabled during training, or the subsequent inference phase can be performed with \texttt{inferenceStartGenerateGlobalFeatureNeuronsTensor=True} to regenerate the global feature neurons. Otherwise the inference phase will reinitialise the global feature neurons as all zeros. This is sufficient as the inference phase currently only uses the activation values of the global feature neurons. 

The database global feature neurons are by default stored on device GPU during training (\texttt{useGPUsparse=True}), and CPU during inference (\texttt{useGPUsparse=False}). The per sequence feature neurons accessed during training (if \texttt{storeDatabaseGlobalFeatureNeuronsInRam=False}) are stored on the same device.

\subparagraph{Database feature connections}
Database feature connections sparse tensors are by default all stored in RAM during training (\texttt{storeDatabaseFeatureConnectionsAndColumnFeatureNeuronsInRam=True}). Else the filesystem database feature connections are written/read into/from RAM on a per sequence basis (RAM memory discarded).

The RAM database feature connections are always stored on device CPU during benchmarking (i.e. \texttt{useGPUdatabase=False}). The per sequence feature connections are by default moved to device GPU during training (i.e. \texttt{useGPUsparse=True}), and moved to (kept on) device CPU during inference (i.e. \texttt{useGPUsparse=False}).

\paragraph{Filesystem}

The filesystem database is stored on a locally connected SSD drive.

\paragraph{Tables}

\Cref{tab:oscar-performance-storeDatabaseInRamTrue} contains OSCAR-2201 performance measurements for different execution phases (train, eval training-set, eval test-set) and training sizes, with store database feature neurons and connections actively in RAM (\texttt{storeDatabaseGlobalFeatureNeuronsInRam=True}, \texttt{storeDatabaseFeatureConnectionsAndColumnFeatureNeuronsInRam=True}).

\Cref{tab:oscar-performance-storeDatabaseInRamFalse} contains OSCAR-2201 performance measurements for different execution phases (train, eval training-set, eval test-set) and training sizes, without store database feature connections actively in RAM (\texttt{storeDatabaseFeatureConnectionsAndColumnFeatureNeuronsInRam=False}).

\Cref{tab:oscar-performance-param} contains OSCAR-2201 model paramaters for different training sizes.

\Cref{tab:oscar-performance-trainSparseConnectionsTensor} contains OSCAR-2201 training performance measurements for different train phase feature connection tensor type (dense/sparse; \texttt{trainSparseConnectionsTensor=True/False}), and training sizes, with train phase feature neuron tensor type dense (\texttt{trainSparseNeuronsTensor=False}), and store database feature neurons and connections actively in RAM (\texttt{storeDatabaseGlobalFeatureNeuronsInRam=True}, \texttt{storeDatabaseFeatureConnectionsAndColumnFeatureNeuronsInRam=True}).

\Cref{tab:oscar-performance-trainGlobalOrColumnFeatureNeuronTensors} contains OSCAR-2201 training performance measurements for different store database feature neurons location (global feature neurons tensor, independent tensors for each column; \texttt{storeDatabaseGlobalFeatureNeuronsInRam=True/False}) and training sizes, with store database feature neurons and connections actively in RAM (\texttt{storeDatabaseFeatureConnectionsAndColumnFeatureNeuronsInRam=True}).

\Cref{tab:oscar-performance-trainSparseConnectionsNeuronsTensor-storeDatabaseFeatureNeuronsInRamFalse} contains OSCAR-2201 training performance measurements for different train phase feature neuron tensor types (\texttt{trainSparseNeuronsTensor=True/False}) and connection tensor types (\texttt{trainSparseConnectionsTensor=True/False}); dense or sparse, tensor device (GPU/CPU DDR4; \texttt{useGPUsparse=True/False}), and training sizes, with store database feature neurons and connections actively in RAM (\texttt{storeDatabaseFeatureConnectionsAndColumnFeatureNeuronsInRam=True}), and feature neuron sparse tensors stored independently for each column (\texttt{storeDatabaseGlobalFeatureNeuronsInRam=False}). The device indicated is assigned to both tensor types; sparse/dense (\texttt{useGPUsparse}/\texttt{useGPUdense}).

\Cref{tab:oscar-performance-trainSparseConnectionsNeuronsTensor-storeDatabaseFeatureNeuronsInRamFalse-storeDatabaseFeatureConnectionsAndColumnFeatureNeurons-DDR4} contains OSCAR-2201 training performance measurements for different database feature connections active storage location (DDR4 RAM/filesystem; \texttt{storeDatabaseFeatureConnectionsAndColumnFeatureNeuronsInRam=True/False}) and training sizes, with train phase feature neuron and connection sparse tensors (\texttt{trainSparseNeuronsTensor=True}, \texttt{trainSparseConnectionsTensor=True}), sparse tensor device CPU (\texttt{useGPUsparse=False}), and feature neuron sparse tensors stored independently for each column (\texttt{storeDatabaseGlobalFeatureNeuronsInRam=False}). The device indicated is assigned to both tensor types; sparse/dense (\texttt{useGPUsparse=False}/\texttt{useGPUdense=False}).

\Cref{tab:oscar-performance-trainSparseConnectionsNeuronsTensor-storeDatabaseFeatureNeuronsInRamFalse-storeDatabaseFeatureConnectionsAndColumnFeatureNeurons-DDR5} contains OSCAR-2201 training performance measurements for different database feature connections active storage location (DDR5 RAM/filesystem; \texttt{storeDatabaseFeatureConnectionsAndColumnFeatureNeuronsInRam=True/False}) and training sizes, with train phase feature neuron and connection sparse tensors (\texttt{trainSparseNeuronsTensor=True}, \texttt{trainSparseConnectionsTensor=True}), sparse tensor device CPU (\texttt{useGPUsparse=False}), and feature neuron sparse tensors stored independently for each column (\texttt{storeDatabaseGlobalFeatureNeuronsInRam=False}). The device indicated is assigned to both tensor types; sparse/dense (\texttt{useGPUsparse=False}/\texttt{useGPUdense=False}).

\Cref{tab:oscar-performance-storeDatabaseFeatureNeuronsInRamFalse-storeDatabase} contains OSCAR-2201 training performance measurements for different database feature connections active storage location (RAM/filesystem; \texttt{storeDatabaseFeatureConnectionsAndColumnFeatureNeuronsInRam=True/False}) and training sizes, with train phase feature neuron and connection dense tensors (\texttt{trainSparseNeuronsTensor=False}, \texttt{trainSparseConnectionsTensor=False}), dense tensor device GPU (\texttt{useGPUdense=True}), and feature neuron sparse tensors stored independently for each column (\texttt{storeDatabaseGlobalFeatureNeuronsInRam=False}).

\begin{table}[H]
\centering
\caption{OSCAR-2201 performance measurements for different execution phases (train, eval training-set, eval test-set) and training sizes, with store database feature connections actively in RAM}
...
\midrule
8 & 3 & 5 & 5,000       & train & 00:01:05 & 00:00:54 & 00:00:11 & 0.9GB & 2.1GB \\
8 & 3 & 5 & 200,000     & train & 01:30:38 & 01:26:02 & 00:04:36 & 1.9GB & 8.0GB \\
%8 & 3 & 5 & 1,000,000   & train & 16:00:00 & 15:38:24 & 00:21:36 & 7.0GB & >20.6GB \\
8 & 3 & 5 & 5,000       & tr eval & 00:00:39 & 00:00:22 & 00:00:17 & 0.0GB & 1.6GB \\
8 & 3 & 5 & 200,000     & tr eval & 00:18:11 & 00:10:15 & 00:07:55 & 0.4GB & 7.6GB \\
%8 & 3 & 5 & 1,000,000   & tr eval & 01:24:03 & 00:40:29 & 00:43:33 & 1.5GB & >23.3GB \\
8 & 3 & 5 & 5,000       & ts eval & 00:00:57 & 00:00:37 & 00:00:20 & 0.0GB & 1.7GB \\
8 & 3 & 5 & 200,000     & ts eval & 00:25:42 & 00:15:23 & 00:10:19 & 0.4GB & 8.1GB \\
...
%\begin{table}[H]
%\centering
%\caption{OSCAR-2201 DDR5 performance measurements for different execution phases (train, eval training-set, eval test-set) and training sizes, with store database feature connections actively in RAM}
%\label{tab:oscar-performance-storeDatabaseInRamTrue}
%\begin{tabular}{rrrrrrrrrrrrr}
%\toprule
%\textbf{s} & \textbf{c} & \textbf{f} & \textbf{sequences} & \textbf{phase} & \textbf{t tot} & \textbf{t proc} & \textbf{t w/r} & \textbf{gpu ram} & \textbf{cpu ram} \\
%\midrule
%8 & 3 & 5 & 5,000       & train & 00:01:00 & 00:00:53 & 00:00:07 & 0.9GB & 2.1GB \\
%8 & 3 & 5 & 200,000     & train & 01:25:37 & 01:22:13 & 00:03:24 & 1.9GB & 8.0GB \\
%8 & 3 & 5 & 5,000       & tr eval & 00:00:39 & 00:00:21 & 00:00:17 & 0.0GB & 1.6GB \\
%8 & 3 & 5 & 200,000     & tr eval & 00:17:12 & 00:09:17 & 00:07:55 & 0.4GB & 7.6GB \\
%8 & 3 & 5 & 5,000       & ts eval & 00:00:50 & 00:00:33  & 00:00:17 & 0.0GB & 1.7GB \\
%8 & 3 & 5 & 200,000     & ts eval & 00:22:35 & 00:14:29 & 00:08:05 & 0.4GB & 8.1GB \\
%\bottomrule
%\end{tabular}
%\end{table}
...
\begin{table}[H]
\centering
\caption{OSCAR-2201 performance measurements for different execution phases (train, eval training-set, eval test-set) and training sizes, without store database feature connections actively in RAM}
...
\midrule
8 & 3 & 5 & 5,000       & train & 00:01:19 & 0.9GB & 1.9GB \\
8 & 3 & 5 & 200,000     & train & 02:00:17 & 2.0GB & 2.5GB \\
8 & 3 & 5 & 5,000       & tr eval & 00:00:23 & 0.0GB & 1.4GB \\
8 & 3 & 5 & 200,000     & tr eval & 00:09:50 & 0.4GB & 2.7GB \\
8 & 3 & 5 & 5,000       & ts eval & 00:00:30 & 0.0GB & 1.5GB \\
8 & 3 & 5 & 200,000     & ts eval & 00:15:13 & 0.4GB & 3.0GB \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{rrrrrrrrr}
\toprule
\textbf{s} & \textbf{c} & \textbf{f} & \textbf{sequences} & \textbf{neurons} & \textbf{connections} & \textbf{columns} & \textbf{features} & \textbf{db size} \\
\midrule
8 & 3 & 5 & 5,000       & 60,364 & 2,150,611 & 9,964 & 6,263 & 0.2GB \\
8 & 3 & 5 & 200,000     & 1,382,140 & 76,581,459 & 203,804 & 43,636 & 6.0GB \\
8 & 3 & 5 & 1,000,000   & 5,274,133 & 353,715,670 & 795,326 & 113,756 & 25.5GB \\
8 & 3 & 5 & 3,000,000   & 12,963,467 & 986,063,778 & 2,035,793 & 235,761 & 69.4GB \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{OSCAR-2201 training performance measurements for different train phase feature connection tensor type (dense/sparse), and training sizes, with train phase feature neuron tensor type dense, and store database feature connections actively in RAM}
\label{tab:oscar-performance-trainSparseConnectionsTensor}
\begin{tabular}{rrrrrrrrrrrrr}
\toprule
\textbf{s} & \textbf{c} & \textbf{f} & \textbf{sequences} & \textbf{type} & \textbf{t tot} & \textbf{t proc} & \textbf{t w/r} & \textbf{gpu ram} & \textbf{cpu ram} \\
\midrule
8 & 3 & 5 & 5,000       & dense & 00:01:05 & 00:00:54 & 00:00:11 & 0.9GB & 2.1GB \\
8 & 3 & 5 & 200,000     & dense & 01:30:38 & 01:26:02 & 00:04:36 & 1.9GB & 8.0GB \\
8 & 3 & 5 & 5,000       & sparse con & 00:02:03 & 00:01:47 & 00:00:16 & 0.1GB & 2.0GB \\
8 & 3 & 5 & 200,000     & sparse con & 01:52:59 & 01:48:03 & 00:04:56 & 1.8GB & 8.0GB \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{OSCAR-2201 training performance measurements for different store database feature neurons location (global feature neurons tensor, independent tensors for each column) and training sizes, with store database feature neurons and connections actively in RAM}
\label{tab:oscar-performance-trainGlobalOrColumnFeatureNeuronTensors}
\begin{tabular}{rrrrrrrrrrrrr}
\toprule
\textbf{s} & \textbf{c} & \textbf{f} & \textbf{sequences} & \textbf{method} & \textbf{t tot} & \textbf{t proc} & \textbf{t w/r} & \textbf{gpu ram} & \textbf{cpu ram} \\
\midrule
8 & 3 & 5 & 5,000       & global & 00:01:05 & 00:00:54 & 00:00:11 & 0.9GB & 2.1GB \\
8 & 3 & 5 & 200,000     & global & 01:30:38 & 01:26:02 & 00:04:36 & 1.9GB & 8.0GB \\
8 & 3 & 5 & 5,000       & column & 00:02:16 & 00:01:54 & 00:00:21 & 0.9GB & 2.1GB \\
8 & 3 & 5 & 200,000     & column & 00:52:38 & 00:48:53 & 00:03:45 & 1.4GB & 8.0GB \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{OSCAR-2201 training performance measurements for different train phase feature neuron and connection tensor types (dense/sparse), tensor device (GPU/CPU DDR4), and training sizes, with store database feature neurons and connections actively in RAM, and feature neuron sparse tensors stored independently for each column}
\label{tab:oscar-performance-trainSparseConnectionsNeuronsTensor-storeDatabaseFeatureNeuronsInRamFalse}
\begin{tabular}{rrrrrrrrrrrrrr}
\toprule
\textbf{s} & \textbf{c} & \textbf{f} & \textbf{sequences} & \textbf{type} & \textbf{device} & \textbf{t tot} & \textbf{t proc} & \textbf{t w/r} & \textbf{gpu ram} & \textbf{cpu ram} \\
\midrule
8 & 3 & 5 & 5,000       & dense & GPU & 00:02:16 & 00:01:54 & 00:00:21 & 0.9GB & 2.1GB \\
8 & 3 & 5 & 200,000     & dense & GPU & 00:52:38 & 00:48:53 & 00:03:45 & 1.4GB & 8.0GB \\
8 & 3 & 5 & 5,000       & sparse & GPU & 00:01:33 & 00:01:23 & 00:00:09 & 0.02GB & 2.0GB \\
8 & 3 & 5 & 200,000     & sparse & GPU & 00:58:28 & 00:54:55 & 00:03:33 & 0.5GB & 9.7GB \\
8 & 3 & 5 & 5,000       & sparse & CPU & 00:01:16 & 00:01:07 & 00:00:08 & 0.0GB & 1.8GB \\
8 & 3 & 5 & 200,000     & sparse & CPU & 01:01:01 & 00:57:33 & 00:03:27 & 0.0GB & 8.2GB \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{OSCAR-2201 training performance measurements for different database feature connections active storage location (DDR4 RAM/filesystem) and training sizes, with train phase feature neuron and connection sparse tensors, sparse tensor device CPU, and feature neuron sparse tensors stored independently for each column}
\label{tab:oscar-performance-trainSparseConnectionsNeuronsTensor-storeDatabaseFeatureNeuronsInRamFalse-storeDatabaseFeatureConnectionsAndColumnFeatureNeurons-DDR4}
\begin{tabular}{rrrrrrrrrrrrr}
\toprule
\textbf{s} & \textbf{c} & \textbf{f} & \textbf{sequences} & \textbf{database} & \textbf{t tot} & \textbf{t proc} & \textbf{t w/r} & \textbf{gpu ram} & \textbf{cpu ram} \\
\midrule
8 & 3 & 5 & 5,000       & RAM & 00:01:16 & 00:01:07 & 00:00:08 & 0.0GB & 1.8GB \\
8 & 3 & 5 & 200,000     & RAM & 01:01:01 & 00:57:33 & 00:03:27 & 0.0GB & 8.2GB \\
8 & 3 & 5 & 5,000       & filesys & 00:02:07 & 00:02:07 & 00:00:00 & 0.0GB & 1.7GB \\
8 & 3 & 5 & 200,000     & filesys & 01:34:21 & 01:34:21 & 00:00:00 & 0.0GB & 2.0GB \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{OSCAR-2201 training performance measurements for different database feature connections active storage location (DDR5 RAM/filesystem) and training sizes, with train phase feature neuron and connection sparse tensors, sparse tensor device CPU, and feature neuron sparse tensors stored independently for each column}
\label{tab:oscar-performance-trainSparseConnectionsNeuronsTensor-storeDatabaseFeatureNeuronsInRamFalse-storeDatabaseFeatureConnectionsAndColumnFeatureNeurons-DDR5}
\begin{tabular}{rrrrrrrrrrrrr}
\toprule
\textbf{s} & \textbf{c} & \textbf{f} & \textbf{sequences} & \textbf{database} & \textbf{t tot} & \textbf{t proc} & \textbf{t w/r} & \textbf{gpu ram} & \textbf{cpu ram} \\
\midrule
8 & 3 & 5 & 5,000       & RAM & 00:00:34 & 00:00:28 & 00:00:06 & 0.0GB & 1.8GB \\
8 & 3 & 5 & 200,000     & RAM & 00:30:58 & 00:28:38 & 00:02:19 & 0.0GB & 8.2GB \\
8 & 3 & 5 & 5,000       & filesys & 00:00:49 & 00:00:49 & 00:00:00 & 0.0GB & 1.7GB \\
8 & 3 & 5 & 200,000     & filesys & 00:56:58 & 00:56:58 & 00:00:00 & 0.0GB & 2.0GB \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{OSCAR-2201 training performance measurements for different database feature connections active storage location (RAM/filesystem) and training sizes, with train phase feature neuron and connection dense tensors, dense tensor device GPU, and feature neuron sparse tensors stored independently for each column}
\label{tab:oscar-performance-storeDatabaseFeatureNeuronsInRamFalse-storeDatabase}
\begin{tabular}{rrrrrrrrrrrrr}
\toprule
\textbf{s} & \textbf{c} & \textbf{f} & \textbf{sequences} & \textbf{database} & \textbf{t tot} & \textbf{t proc} & \textbf{t w/r} & \textbf{gpu ram} & \textbf{cpu ram} \\
\midrule
8 & 3 & 5 & 5,000       & RAM & 00:01:04 & 00:00:55 & 00:00:09 & 0.9GB & 2.1GB \\
8 & 3 & 5 & 200,000     & RAM & 00:41:04 & 00:37:28 & 00:03:35 & 1.4GB & 8.0GB \\
8 & 3 & 5 & 1,000,000   & RAM & 03:44:37 & 03:25:47 & 00:18:50 & 2.9GB & 34.1GB \\
8 & 3 & 5 & 5,000       & filesys & 00:01:29 & 00:01:29 & 00:00:00 & 0.9GB & 1.9GB \\
8 & 3 & 5 & 200,000     & filesys & 01:10:29 & 01:10:29 & 00:00:00 & 1.1GB & 2.2GB \\
8 & 3 & 5 & 1,000,000   & filesys & 08:04:29 & 08:04:29 & 00:00:00 & 2.1GB & 3.0GB \\
\bottomrule
\end{tabular}
\end{table}
...
An additional set of accuracy measurements were taken with GIAANN Proto 2j.

A number of changes to the default settings were introduced in GIAANN Proto 2 that affect evaluation accuracies:

\begin{itemize}
 	\item Implement option tokeniserSubword - a modern BPE/SentencePiece subword tokeniser (\texttt{o200k_base} encoding of the OpenAI \texttt{tiktoken} library).
	...
	\item add option \texttt{useBenchmarkDefaultsEvalTestSetOptim} - set \texttt{inferenceSegmentTiming = "none"}, \texttt{inferenceConnectionsStrengthBoolean = False} (and \texttt{inferenceActivationsType = "boolf"}) by default during test-set evaluation.
\end{itemize}
...
\Cref{tab:protov2j-oscar-train-top1-accuracy} contains Proto v2 OSCAR-2201 Top-1 training-set accuracy for different numbers training sizes, with targets fed as input.
...
\begin{table}[H]
\centering
\caption{Proto v2 OSCAR-2201 Top-1 training-set accuracy for different numbers training sizes, with targets fed as input.}
...
\Cref{tab:protov2j-oscar-test-top1-accuracy-spacyPipelineOptimisations-inferenceConnectionStrengthTypes} contains Proto v2 OSCAR-2201 Top-1 test-set accuracy for different inference connection strength types, training sizes, and inference activations, with segment timing none (t-1 none), and targets fed as input. The different inference connection strength types used were ``bool'' (\texttt{inferenceConnectionsStrengthBoolean=True}) and ``float'' (\texttt{inferenceConnectionsStrengthBoolean=False}). The default inference activation type used during proto v2 benchmarking is still ``bool f''. The default strength type used during proto v2 benchmarking is ``float''. The table also includes the total number of trained words and tokens. 
...
\begin{table}[H]
\centering
\caption{Proto v2 OSCAR-2201 Top-1 test-set accuracy for different inference connection strength types, training sizes, and inference activations, with segment timing none (t-1 none), and targets fed as input.}
...
An additional set of performance measurements were taken with GIAANN Proto 2j.
...
\Cref{tab:protov2j-oscar-performance} contains Proto v2 OSCAR-2201 performance measurements for different execution phases (train, eval test-set) and training sizes and different training sizes, with different database feature connections active storage location for train/eval (DDR5 RAM/filesystem), with train phase feature neuron and connection sparse tensors, sparse tensor device CPU, and feature neuron sparse tensors stored independently for each column. Train times were measured cumulatively > 1M sequences.

\begin{table}[H]
\centering
\caption{Proto v2 OSCAR-2201 performance measurements for different execution phases (train, eval test-set) and training sizes and different training sizes, with different database feature connections active storage location for train/eval (DDR5 RAM/filesystem), with train phase feature neuron and connection sparse tensors, sparse tensor device CPU, and feature neuron sparse tensors stored independently for each column.}
