...
\Cref{tab:wiki-train-top1-accuracy} contains Wikipedia top-1 training-set accuracy for different numbers of segments and training budgets, with targets/predictions fed as input, and segment timing exact (``t-1 exact''). It includes `t-1 exact pred' \\ (\texttt{inferenceUseNextTokenPredictionsOrTargetsToActivateNextColumnFeatures=True}), a non-standard top-1 accuracy report that is more sensitive to errors and can therefore better differentiate between ablations (it feeds current predictions rather than current targets to input).
...
\begin{table}[H]
\centering
\caption{OSCAR-2201 Top-1 training-set accuracy for different numbers of dendritic branches and time constraints, with targets fed as input.}
\label{tab:oscar-train-top1-accuracy-multipleDendriticBranches}
...
\midrule
8 & 3 & 5 & 5,000     & 2 uniq & 0.941 & 0.856 & 0.973 & 0.940 & 2.2M & 0.1GB \\
8 & 3 & 5 & 200,000   & 2 uniq & 0.775 & 0.843 & 0.894 & 0.853 & 76.6M & 4.5GB \\
8 & 3 & 5 & 5,000     & 5 rand & 0.941 & 0.924 & 0.973 & 0.938 & 2.1M & 0.1GB \\
8 & 3 & 5 & 200,000   & 5 rand & 0.778 & 0.917 & 0.893 & 0.855 & 76.8M & 4.6GB \\
8 & 3 & 5 & 5,000     & 10 rand & 0.941 & 0.924 & 0.973 & 0.965 & 2.1M & 0.1GB \\
8 & 3 & 5 & 200,000   & 10 rand & 0.778 & 0.915 & 0.894 & 0.790 & 76.8M & 4.6GB \\
\bottomrule
\end{tabular}
\end{table}
...
To improve evaluation efficency (speed), \texttt{spacyPipelineOptimisations=True} has been set in \Cref{tab:oscar-test-top1-accuracy-spacyPipelineOptimisations}, \Cref{tab:oscar-test-top1-accuracy-spacyPipelineOptimisations-trainsize}, \Cref{tab:oscar-test-top1-accuracy-useBenchmarkDefaultsFalse-multisentencePredictions}, \Cref{tab:oscar-test-top1-accuracy-spacyPipelineOptimisations-inferenceActivations}, and \Cref{tab:oscar-test-top1-accuracy-spacyPipelineOptimisations-inferenceActivations-multisentencePredictionsInference}, as these optimisations have been confirmed to not significantly affect reported test-set accuracy. With 5000 trained sequences for example, the test-set accuracy difference with \texttt{spacyPipelineOptimisations=True} is exactly -0.002, for seed lengths 8, 12, and 16.
...
\begin{table}[H]
\centering
\caption{OSCAR-2201 Top-1 test-set accuracy for different numbers of segments, with segment timing biased (t-1 bias), and targets fed as input.}
...
16 & 5 & 9 & 5,000    & 0.023 & 0.023 & 0.024 & 28.9M & 0.2GB \\
...
\begin{table}[H]
\centering
\caption{OSCAR-2201 Top-1 test-set accuracy for different training sizes, and different inference activations, with segment timing none (t-1 none), targets fed as input, and multiple sentence predictions (during test-set only).}
...
\begin{table}[H]
\centering
\caption{OSCAR-2201 Top-1 test-set accuracy for different numbers of dendritic branches and time constraints, with targets fed as input.}
\label{tab:oscar-test-top1-accuracy-multipleDendriticBranches}
\begin{tabular}{rrrrrrrrrrrrr}
\toprule
\textbf{s} & \textbf{c} & \textbf{f} & \textbf{sequences} & \textbf{dendrites} & \textbf{t1 bia} & \textbf{t1 exa} & \textbf{t1 non} & \textbf{int f+c}  & \textbf{param} & \textbf{db size} \\
\midrule
