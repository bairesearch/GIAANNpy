...
GIAANN offers advantages in interpretability, incremental learning, and energy efficiency.
...
The prediction performance of GIAANN appears lower than that of a Transformer. GIAANN proto v1 produced an OSCAR-2201 whole-word token top-1 test accuracy $\approx 0.15$ after 3M sequences comprising 70M words---equivalent to 80M tokens (\Cref{tab:oscar-test-top1-accuracy-spacyPipelineOptimisations-trainsize}), whereas a 6-layer 512-token context RoBERTa Causal Transformer Language Model \citep{liu2019roberta} can deliver a subword token top-1 test accuracy $\approx 0.3$ after 1M sequences comprising  $\approx$ 333M tokens \citep{bairesearch2022TSBNLP}. For the OSCAR-2201 web crawl text (`oscar-corpus/OSCAR-2201' `en' split) a subword prediction accuracy of $\approx 0.3$ translates to a whole word-accuracy of $\approx 0.17$ to $\approx 0.24$, which is above what was measured by GIAANN proto v1. GIAANN proto v2 produced an OSCAR-2201 subword token top-1 test accuracy $\approx 0.21$ after $\approx$ 1M sequences comprising 66M tokens (\Cref{tab:protov2m-oscar-test-top1-accuracy-inferenceLeakyIntegrateAndFire}), compared to the RoBERTa test accuracy of $\approx 0.3$ after $\approx$ 333M tokens.
