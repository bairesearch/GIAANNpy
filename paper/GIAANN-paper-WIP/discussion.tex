...
While GIAANN might not reach the performance of a Transformer on benchmark datasets and evaluation metrics, it offers advantages in interpretability, incremental learning, and energy efficiency. Likewise, experiments are indicative of consistent scaling. GIAANN produced an OSCAR-2201 whole-word token top-1 test accuracy $\approx 0.15$ after 1M samples (\Cref{tab:oscar-test-top1-accuracy-spacyPipelineOptimisations-trainsize}), whereas a RoBERTa Causal Transformer Language Model typically delivers a subword token top-1 test accuracy $\approx 0.3$ after 1M samples \citep{bairesearch2022TSBNLP}.
...
And whereas Transformers require enormous training data and then struggle with one-shot or continual learning, GIAANN can potentially incorporate new knowledge by adjusting a few connections in the semantic graph or adding a new neuron/column, without retraining the whole network. 
