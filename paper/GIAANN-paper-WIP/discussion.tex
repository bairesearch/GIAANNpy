...
The prediction performance of GIAANN is lower than that of a Transformer, however experiments are indicative of consistent scaling.
GIAANN produced an OSCAR-2201 whole-word token top-1 test accuracy $\approx 0.15$ after 1M samples (\Cref{tab:oscar-test-top1-accuracy-spacyPipelineOptimisations-trainsize}), whereas a RoBERTa Causal Transformer Language Model can deliver a subword token top-1 test accuracy $\approx 0.3$ after 1M samples \citep{bairesearch2022TSBNLP}.
For the OSCAR-2201 web crawl text (`oscar-corpus/OSCAR-2201' `en' split) a subword prediction accuracy of $\approx 0.3$ translates to a whole word-accuracy of $\approx 0.17$ to $\approx 0.24$, which is above what is measured by GIAANN.
...
GIAANN is also implemented using whole-word tokenisation, which limits generalistion to new test set words not seen in the training set.
