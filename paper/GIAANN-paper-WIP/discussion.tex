...
The prediction performance of GIAANN appears lower than that of a Transformer, however experiments are indicative of consistent scaling.

GIAANN proto v1 produced an OSCAR-2201 whole-word token top-1 test accuracy $\approx 0.15$ after 3M sequences comprising 70M words---equivalent to 80M tokens (\Cref{tab:oscar-test-top1-accuracy-spacyPipelineOptimisations-trainsize}), whereas a 6-layer 512-token context RoBERTa Causal Transformer Language Model \citep{liu2019roberta} can deliver a subword token top-1 test accuracy $\approx 0.3$ after 1M sequences comprising  $\approx$ 500M tokens \citep{bairesearch2022TSBNLP}.
For the OSCAR-2201 web crawl text (`oscar-corpus/OSCAR-2201' `en' split) a subword prediction accuracy of $\approx 0.3$ translates to a whole word-accuracy of $\approx 0.17$ to $\approx 0.24$, which is above what was measured by GIAANN proto v1.

GIAANN proto v2 produced an OSCAR-2201 subword token top-1 test accuracy $\approx 0.26$ after 3M sequences comprising 80M tokens (\Cref{tab:protov2j-oscar-test-top1-accuracy-spacyPipelineOptimisations-inferenceConnectionStrengthTypes}), whereas a 6-layer 512-token context RoBERTa Causal Transformer Language Model can deliver a subword token top-1 test accuracy $\approx 0.3$ after 1M sequences comprising  $\approx$ 500M tokens \citep{bairesearch2022TSBNLP}. While the GIAANN proto v2 benchmarks produce more similar results to the transformer architecture when extrapolated to the same number of input tokens, they still contain a number of constraints that can artificially enhance their peformance (e.g. single-sentence sequence prediction, discarding of sequence concept column segmentation errors). RoBERTa is likewise not a SOTA (state-of-the-art) transformer implementation.

GIAANN also offers advantages in interpretability, incremental learning, and energy efficiency.
...
Whereas Transformers require enormous training data and then struggle with one-shot or continual learning, GIAANN can potentially incorporate new knowledge by adjusting a few connections in the semantic graph or adding a new neuron/column, without retraining the whole network. 
...
Likewise, GIAANN currently supports the dynamic growing of columns and features (not only connections), which appears biologically unrealistic for a cortical substrate. It was developed under the assumption however of a dedicated pool of neurons distributed across columns that is progressively programmed to act as references to newly declared concepts and features (see Appendix~\ref{subsec:appendix-giaann-development}).

Inhibitory interneurons are also critical in cortex for shaping activity; GIAANN currently emphasises excitatory drives with simplified global inhibition or as a mechanism for restricting prediction targets (exclusive top-1 selection, akin to winner-take-all).
...
GIAANN (proto v1 as primarily described in this paper) was also first implemented using whole-word tokenisation, which limited generalistion to new test set words not seen in the training set.

\subsection{Conclusion}

In summary, GIAANN represents a novel synthesis that addresses shortcomings in prior art: it tackles the scaling issue of Hopfield like memories by structuring them into columns; it improves interpretability and incremental learning relative to end-to-end deep nets; it increases neuron computational capacity instead of sheer quantity, aligning with energy constraints; and it grounds neural processing in explicit semantics, narrowing the gap between sub-symbolic neural nets and symbolic reasoning.
...
As such, GIAANN differs from prior models: it is neither purely a Hopfield net, nor an HTM, nor a standard ANN, but an amalgam that leverages their independent strengths.
