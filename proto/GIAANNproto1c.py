...
if(useInference and inferencePredictiveNetwork):
	...
	def nextWordPredictionMLPcreate():
		...
		nextWordPredictionModel = NextWordPredictionMLPmodel(s, c, f, p)
	...
	def nextWordPredictionMLPtrainStep(model, inputs, targets, optimizer, criterion):
		...
		with torch.no_grad():
			...
			if(multipleTargets):
				...
				probs = torch.sigmoid(outputs)  # Shape: (batch_size, c, f)
			else:
				probs = outputs
			
			column_probs = probs.mean(dim=2)  # Shape: (batch_size, c)
			_, concept_columns_indices_next = torch.topk(column_probs, kc, dim=1)
			...
			for column_idx in concept_columns_indices_next:
				...
				topk_feature_probs, topk_feature_indices = torch.topk(feature_probs, kf)  # Shapes: (kf,), (kf,)
				top_kf_indices.append(topk_feature_indices)

			concept_columns_feature_indices_next = torch.stack(top_kf_indices)  # Shape: (batch_size, kc, kf)
...
def process_concept_words_inference(sequence_observed_columns, doc, doc_seed, doc_predict, num_seed_tokens, num_prediction_tokens):
	...
	concept_mask = torch.tensor([i in sequence_observed_columns.columns_index_sequence_word_index_dict for i in range(len(lemmas_doc))], dtype=torch.bool)
	...
	for wordPredictionIndex in range(num_prediction_tokens):
		...
		featurePredictionTargetMatch, concept_columns_indices, concept_columns_feature_indices = process_column_inference_prediction(observed_columns_dict, wordPredictionIndex, sequenceWordIndex, words_doc, lemmas_doc, concept_columns_indices, concept_columns_feature_indices, concept_mask, sequence_observed_columns.columns_index_sequence_word_index_dict)
...
def process_column_inference_prediction(observed_columns_dict, wordPredictionIndex, sequenceWordIndex, words_doc, lemmas_doc, concept_columns_indices, concept_columns_feature_indices, concept_mask, columns_index_sequence_word_index_dict):
	...
	if(wordPredictionIndex > 0):
		...
		if(deactivateNeuronsUponPrediction):
			if(useSANI):
				...
				segment_indices = torch.zeros_like(concept_columns_indices)
				indices_to_update = torch.stack([segment_indices, concept_columns_indices, concept_columns_feature_indices.squeeze(dim=0)], dim=0)
				global_feature_neurons_activation = modify_sparse_tensor(global_feature_neurons_activation, indices_to_update, 0)
	...
	if(inferencePredictiveNetwork):
		concept_columns_indices_next, concept_columns_feature_indices_next, kc = predictMostActiveFeature(global_feature_neurons_activation, words_doc, lemmas_doc, wordPredictionIndex, sequenceWordIndex, concept_mask, columns_index_sequence_word_index_dict)	
	else:
...
def predictMostActiveFeature(global_feature_neurons_activation, words_doc, lemmas_doc, wordPredictionIndex, sequenceWordIndex, concept_mask, columns_index_sequence_word_index_dict):	
	...
	concept_columns_indices_next, concept_columns_feature_indices_next = nextWordPredictionMLPtrainStep(nextWordPredictionModel, global_feature_neurons_activation, targets, optimizer, criterion)
...
def process_features(start_indices, end_indices, doc, words, lemmas, pos_tags, sequence_observed_columns, concept_indices, train, num_seed_tokens=None, numberConceptsInSeed=None):
	...
	if(useSANI):
		...
		feature_neurons_segment_mask = torch.ones((cs, array_number_of_segments), dtype=array_type)
			...
			for j in range(start_indices[i], end_indices[i]):	#sequence word index
				...
				if(j in sequence_observed_columns.columns_index_sequence_word_index_dict):	#test is required for concept neurons
					...
					if(useSANI):
						...
						feature_neurons_active[0, sequence_feature_index] = 1
...
def createFeatureConnectionsActive(feature_neurons_active, cs, fs, columns_word_order, feature_neurons_word_order):
	...
	if(useSANI):
		...
		feature_connections_segment_mask = torch.ones_like(feature_connections_active)
